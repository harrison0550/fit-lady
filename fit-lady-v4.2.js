...
function coachGreeting(){const h=new Date().getHours();return h<12?"Good morning, Carrie 🌸":h<18?"Good afternoon, Carrie 🌸":"Good evening, Carrie 🌸";}

const APP_VERSION="1.0";
const PROFILE={name:"Carrie",weight:190,bodyFat:null,height:"5\'3\"",age:38,waist:39.5,targetLow:140,targetHigh:145};
const WORKOUTS={
A:[
{name:"Cable Chest Press",scheme:"3 × 10",sets:3,reps:10,rest:75,img:"assets/gym_front.jpeg",label:"Dual D-handles • pulley positions 7 and 7",markers:true,setup:[["Pulley L","7"],["Pulley R","7"],["Attachment","Dual D-handles"],["Bench","Standing or upright"]],cues:["Start light and keep two to three reps in reserve.","Keep ribs down and shoulders relaxed.","Press forward and slightly inward.","Return slowly."]},
{name:"Lat Pulldown",scheme:"3 × 10",sets:3,reps:10,rest:75,img:"assets/gym_front.jpeg",label:"Lat bar • top fixed cable point",setup:[["Pulley","Top fixed point"],["Attachment","Lat bar"],["Bench","Upright"],["Grip","Just outside shoulders"]],cues:["Sit tall.","Pull elbows toward your sides.","Bring the bar toward the upper chest.","Avoid swinging."]},
{name:"Goblet Squat to Bench",scheme:"3 × 8",sets:3,reps:8,rest:75,img:"assets/bench_flat.jpeg",label:"10–15 lb dumbbell • flat bench as target",setup:[["Weight","10–15 lb"],["Bench","Flat"],["Feet","Comfortable stance"],["Depth","Light bench touch"]],cues:["Hold the dumbbell close.","Sit under control.","Keep the whole foot planted.","Stand smoothly."]},
{name:"Seated Cable Row",scheme:"3 × 10",sets:3,reps:10,rest:75,img:"assets/gym_front.jpeg",label:"Straight bar • pulley position 1",setup:[["Pulley","1"],["Attachment","Straight bar"],["Bench","Flat / seated"],["Torso","Tall"]],cues:["Reach forward without collapsing.","Drive elbows behind you.","Pause at the ribs.","Control the return."]},
{name:"Cable Glute Kickback",scheme:"2 × 12 each",sets:2,reps:12,rest:45,img:"assets/ankle.png",label:"Ankle strap • pulley position 1",setup:[["Pulley","1"],["Attachment","Ankle strap"],["Support","Hold upright"],["Range","No back arch"]],cues:["Keep hips square.","Drive the heel backward.","Pause and squeeze.","Use a small controlled range."]},
{name:"Rope Face Pull",scheme:"2 × 12",sets:2,reps:12,rest:60,img:"assets/rope.png",label:"Rope • pulley position 13",setup:[["Pulley","13"],["Attachment","Rope"],["Bench","None"],["Load","Very light"]],cues:["Pull toward eyebrow level.","Separate the rope ends.","Keep neck relaxed.","Do not shrug."]},
{name:"Rower Zone 2",scheme:"10 minutes",sets:1,reps:10,rest:0,img:"assets/gym_wide.jpeg",label:"Easy conversational pace",setup:[["Damper","Low–moderate"],["Stroke rate","18–24"],["Effort","Easy–moderate"],["Duration","10 min"]],cues:["Push with the legs first.","Keep the handle smooth.","Speak in sentences.","Finish with energy left."]}
],
B:[
{name:"Incline Cable Press",scheme:"3 × 10",sets:3,reps:10,rest:75,img:"assets/bench_incline.jpeg",label:"Bench about 30° • pulleys 5 and 5",setup:[["Pulley L","5"],["Pulley R","5"],["Attachment","Dual D-handles"],["Bench","30° incline"]],cues:["Center the bench.","Keep shoulder blades set.","Press up and inward.","Stop before shoulders roll forward."]},
{name:"Single-Arm Cable Row",scheme:"3 × 10 each",sets:3,reps:10,rest:60,img:"assets/gym_front.jpeg",label:"Single D-handle • pulley position 5",setup:[["Pulley","5"],["Attachment","Single D-handle"],["Bench","None"],["Stance","Split stance"]],cues:["Keep hips square.","Pull toward the back pocket.","Do not twist.","Control the reach."]},
{name:"Smith Box Squat",scheme:"3 × 8",sets:3,reps:8,rest:90,img:"assets/smith_hook.jpeg",label:"Smith bar • bench behind • bar only first session",setup:[["Bar","Smith bar"],["Bench","Flat behind"],["Safety","Above deepest position"],["Load","Bar only first time"]],cues:["Place feet slightly forward.","Sit back toward the bench.","Touch lightly.","Drive through the whole foot."]},
{name:"Cable Romanian Deadlift",scheme:"3 × 10",sets:3,reps:10,rest:75,img:"assets/gym_front.jpeg",label:"Straight bar • both pulleys at position 1",setup:[["Pulley L","1"],["Pulley R","1"],["Attachment","Straight bar"],["Load","Light"]],cues:["Push hips backward.","Keep the bar close.","Stop at a hamstring stretch.","Stand by squeezing glutes."]},
{name:"Cable Hip Abduction",scheme:"2 × 12 each",sets:2,reps:12,rest:45,img:"assets/ankle.png",label:"Ankle strap • pulley position 1",setup:[["Pulley","1"],["Attachment","Ankle strap"],["Support","Hold upright"],["Direction","Leg moves outward"]],cues:["Stand tall.","Keep toes forward.","Use a small range.","Avoid leaning."]},
{name:"Standing Rope Crunch",scheme:"2 × 12",sets:2,reps:12,rest:45,img:"assets/rope.png",label:"Rope • high pulley • no floor crunches",setup:[["Pulley","High"],["Attachment","Rope"],["Position","Standing"],["Load","Light"]],cues:["Hold rope beside the head.","Bring ribs toward pelvis.","Keep hips still.","Do not pull with the arms."]},
{name:"Rower Zone 2",scheme:"15 minutes",sets:1,reps:15,rest:0,img:"assets/gym_wide.jpeg",label:"Easy conversational pace",setup:[["Damper","Low–moderate"],["Stroke rate","18–24"],["Effort","Easy–moderate"],["Duration","15 min"]],cues:["Drive with the legs first.","Relax the shoulders.","Use an easy rhythm.","Finish with energy left."]}
]};
const EQUIPMENT=[
...
function loadState(){
 let current=localStorage.getItem("fitladyv4.2");
 if(current)return JSON.parse(current);
 let old=localStorage.getItem("fitlady_old");
 if(old){
   const migrated=JSON.parse(old);
   localStorage.setItem("fitladyv4.2",JSON.stringify(migrated));
   return migrated;
 }
 return {};
}
...
const app=document.getElementById("app");const save=()=>localStorage.setItem("fitladyv4.2",JSON.stringify(s));
...
function home(){
 const last=s.checkins.at(-1),w=last?.weight||PROFILE.weight,waist=last?.waist||PROFILE.waist; const plan=todayPlan(); const label=plan==="A"?"Full Body A":plan==="B"?"Full Body B":plan==="cardio"?"Rower Zone 2":"Recovery Day";
 ...
 <section class="card"><div class="section-head"><div><span class="eyebrow">READINESS CHECK</span><h2>How do you feel?</h2></div><span class="version-badge">V1.0</span></div>
 ...
 <section class="card fitlady-note"><span class="eyebrow">ENERGY & HYDRATION</span><h3>Train by symptoms, not by ego</h3><p class="muted">Because appetite, hydration and energy can vary on Wegovy and with low iron, use lighter loads on tired or nauseated days. Stop for unusual dizziness, faintness, chest discomfort, racing heartbeat or shortness of breath that feels out of proportion.</p></section>
 ...
 <section class="card"><span class="eyebrow">THIS WEEK</span><h2>Training schedule</h2><div class="schedule">
 ${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d,i)=>{const t=["strength","cardio","strength","recovery","strength","cardio","recovery"][i];const icon=t==="strength"?"●":t==="cardio"?"▲":"✓";return `<div class="day ${t} ${i===((new Date().getDay()+6)%7)?"today":""}"><b>${d}</b><span>${icon}</span></div>`}).join("")}</div></section>`;
...
function equipment(){app.innerHTML=`<section class="card"><span class="eyebrow">AVAILABLE EQUIPMENT</span><h2>Carrie's Home Gym</h2><p class="muted">The program only uses equipment you own.</p></section><section class="equip-grid" id="eq"></section>`;const q=document.getElementById("eq");EQUIPMENT.forEach(([n,d,img])=>q.appendChild(elem("article","equip",`<img src="${img}"><div><h3>${n}</h3><p>${d}</p></div>`)))}
...
function progress(){
 ...
 app.innerHTML=`<section class="card"><div class="section-head"><div><span class="eyebrow">PROFILE</span><h2>${PROFILE.name}' Fit Lady</h2></div><span class="version-badge">V1.0</span></div><p class="muted">${PROFILE.height} • age ${PROFILE.age} • target 190–200 lb</p><div class="bar"><span style="width:${goalPct}%"></span></div><p class="muted">${lost>=0?lost.toFixed(1)+" lb lost":Math.abs(lost).toFixed(1)+" lb above starting weight"}</p></section>
 ...
 document.getElementById("exportBtn").onclick=()=>{const blob=new Blob([JSON.stringify(s,null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="fit-lady-backup.json";a.click();URL.revokeObjectURL(a.href)};
 document.getElementById("importInput").onchange=e=>{const f=e.target.files[0];if(!f)return;const reader=new FileReader();reader.onload=()=>{try{const restored=JSON.parse(reader.result);if(!restored||typeof restored!=="object")throw new Error();if(confirm("Replace current app data with this backup?")){s=restored;save();toast("Backup restored");render()}}catch{alert("That file is not a valid Fit Lady backup.")}};reader.readAsText(f)}
}
document.querySelectorAll(".nav").forEach(b=>b.onclick=()=>{s.tab=b.dataset.tab;save();render()});
document.getElementById("reset").onclick=()=>{if(confirm("Reset all app data on this device?")){localStorage.removeItem("fitladyv4.2");localStorage.removeItem("fitlady_old");location.reload()}};
window.addEventListener("error",event=>{
  const target=document.getElementById("app");
  if(target && !target.innerHTML.trim()){
    target.innerHTML=`<section class="card notice"><strong>Fit Lady could not finish loading.</strong><br>Please refresh Safari. Error: ${event.message||"Unknown error"}</section>`;
  }
});
