...
function loadState(){
 let current=localStorage.getItem("fitladyv1");
 if(current)return JSON.parse(current);
 let old=localStorage.getItem("fitlady_old");
 if(old){
   const migrated=JSON.parse(old);
   localStorage.setItem("fitladyv1",JSON.stringify(migrated));
   return migrated;
 }
 return {};
}
...
const app=document.getElementById("app");const save=()=>localStorage.setItem("fitladyv1",JSON.stringify(s));
...
document.getElementById("reset").onclick=()=>{if(confirm("Reset all app data on this device?")){localStorage.removeItem("fitladyv1");localStorage.removeItem("fitlady_old");location.reload()}};
